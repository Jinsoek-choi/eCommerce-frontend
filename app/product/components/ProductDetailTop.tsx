...
export default function ProductDetailTop({ product }: { product: Product }) {
  ...
  const handleLike = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/like/toggle/${product.productId}`, {
        method: "POST",
        credentials: "include",
      });
  ...
  // 상품에 옵션이 실제로 존재하는지 여부
  const hasOption = (product.options?.length ?? 0) > 0;
            ...
            {selectedOptions.map((item) => (
              ...
              >
                <div className="flex-1">
                  <p className="font-medium">{item.optionValue}</p>
